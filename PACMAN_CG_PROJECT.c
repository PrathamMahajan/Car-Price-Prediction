...
void user_input()
{
   if (_kbhit())  // keyboardhit function to take input from the keyboard
   {
      char c1 = _getch();

      if (c1 == -32)  // To ensure that the key pressed is an arrow key
      {
         char c2 = _getch();
       ...
       if((allGhosts[i].position.x==myPacMan.position.x && allGhosts[i].position.y==myPacMan.position.y) || (allGhosts2[i].position.x==myPacMan.position.x && allGhosts2[i].position.y==myPacMan.position.y))
        {
            system("cls");
            printf("\n\n\n");
            printf("\t \t \t \tRUNTIME TERROR\n\t \t \t \t1. Pratham\n\t \t \t \t2. Christina\n\t \t \t \t3. Aiden\n\t \t \t \t4. Derick");
            printf("\n\n\t \t \t \tYOUR SCORE IS:%d",myPacMan.food_coll);
            getchar();
            exit(0);
        }
...
int main()
{
   ...
   while (1)
   {
      ...
      show_playfield();
      check_coll();
      Sleep( 10/ 30 );   //To ensure the speed of the characters
